...
class Zend_Image_Gd 
{
     ...
     * @param int $fromType
     */
    public function __construct($from, $fromType = Zend_Image_Gd::FROM_FILE)
    {
    ...
    */
    ...
    {              
        ...
        return new Zend_Image_Gd($this->im, self::FROM_GD_IMAGE);
    }
    ...
    public function save($to, $quality = 90)
    {
        ...
        //App_File::createDirForFileRecursive($to);
    ...
    public function show($ext, $quality = 90, Zend_Controller_Response_Http $response = null, $headersFilename = null, $mtime = null)
    {
    ...
    */
	...
	public function resizeToSize($nw, $nh)
    {
        if (!$this->im) {
            return $this;
        }
		...
		$resizeImageW = $this->getWidth();
		$resizeImageH = $this->getHeight();
		
		$x = round(($nw - $resizeImageW) / 2);
		$y = round(($nh - $resizeImageH) / 2);
		...
		imagecopyresampled($new,$im,$x,$y,0,0,$resizeImageW,$resizeImageH,$resizeImageW,$resizeImageH);
    ...
    */
    ...
    public function paste($img, $x, $y)
    {
        ...
        if ($img instanceof Zend_Image_Gd) {
            $w = $img->getWidth();
            $h = $img->getHeight();
            $img = &$img->im;
